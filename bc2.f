...
c
c     # Only left boundary is customized to load the data 
c     # from a real shock-wave
c
      ...
      ! Declare additional variables to load left boundary condition
      dimension timeIC(500), pressure(500), p(0:2)

      ! Load common block parameters required (EOS parameters and densities)
...
c     # user-specified boundary conditions go here
      ! Atmospheric pressure and dound speed specified
      p0 = 101325.0
     ...
     & status="old") 
      ...
      ! Look for corresponding value for pressure for current time in data 
      ddt = timeIC(100) - timeIC(101)
      delt = max(ddt,dt)
      do k=1,500
        ! Do a small time translation of the data
        time2 = (timeIC(k) - 0.018758)
        ! Assign corresponding pressure value in Pascals
